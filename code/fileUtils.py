...
class FileUtils(object):
    ...
    def __init__(self, cmd_wrapper):
        if not cmd_wrapper:
            raise Exception("Require cmd_wrapper!")
        # getattr throw exception when "exec_cmd" not found
        if not callable(getattr(cmd_wrapper, "exec_cmd", None)):
            raise Exception("cmd_wrapper's 'exec_cmd' is not callable")
